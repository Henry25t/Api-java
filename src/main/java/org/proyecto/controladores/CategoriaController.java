...
public class CategoriaController {
    ...
    @GetMapping("/lista")
    public ResponseEntity<List<CategoriaSalida>> mostratTodos()
    {
        List<CategoriaSalida> categosias = categoriaService.obtenerTodos();
        if(!categosias.isEmpty())
        {
            return ResponseEntity.ok(categosias);
        }
        return ResponseEntity.notFound().build();
    }
